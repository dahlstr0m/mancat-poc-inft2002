    ...
    return (
      <>
        {this.allProjects
          ...
          .map((
            pj // Maping to display all left projects with PortfolioCard widget & fetching correct corresponding thumbnail for url attrib.
          ) => (
            <PortfolioCard
              projectid={pj.projectid}
              title={pj.title}
              link={'/project/' + pj.projectid}
              imageurl={this.allPosters.find((p) => p.projectid === pj.projectid).url}
            >
              {'ingen data'}
            </PortfolioCard>
          ))}
...
export class ProjectDetails extends Component<{ pathid: number }> {
          ...
          {this.allPosters
            ...
            .map((
              ...
              <PosterCard
                ...
                posterUrl={p.url}
                posterThumbnailUrl={this.allThumbnails
  ...
  mounted() {
     ...
     * Dev-parsing of data
     */
    this.pj = projectsDev.find((pj) => pj.projectid == this.props.pathid);
    this.allPosters = postersDev;
